...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      let errorBlockchain = await self.validateChain();

      if (!errorBlockchain && errorBlockchain.length === 0) {
  ...
  submitStar(address, message, signature, star) {
    let self = this;
    return new Promise(async (resolve, reject) => {
      let time_sent = parseInt(message.split(":")[1]);
      let currentTime = parseInt(new Date().getTime().toString().slice(0, -3));
      let time_elapsed = currentTime - time_sent;
    ...
    return new Promise((resolve, reject) => {
      for (let i = 0; i < self.chain.length; i++) {
        if (self.chain[i].height > 0) {
          if (self.chain[i].getBData().address === address) {
            stars.push(self.chain[i].getBData().data);
          }
        }
      }
      resolve(stars);
